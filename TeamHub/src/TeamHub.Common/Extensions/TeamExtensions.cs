...
namespace TeamAlignment.Core.Common.Extensions
{
    ...
    {
        public static List<Employee> GetEmployees(this Team team)
        {
            using (TeamHubDatabaseContext dataContext = new TeamHubDatabaseContext())
            {
                var employees = dataContext.Transfers.Include(tc => tc.Team)
        ...
        public static List<Leave> GetTimeOff(this Team team)
        {
            using (TeamHubDatabaseContext dataContext = new TeamHubDatabaseContext())
            {
                List<Guid> teamMemberIds = team.GetEmployees().Select(d => d.Id).ToList();
        ...
        public static List<DateTime> GetDevTimeOffDates(this Team team)
        {
            using (TeamHubDatabaseContext dataContext = new TeamHubDatabaseContext())
            {
                ...
                var timeOffs = dataContext.Leaves.Include(to => to.TeamMember)
                                                 .ThenInclude(d => d.Specialization).Where(to => team.GetEmployees().Select(d => d.Id).Contains(to.TeamMemberId)).ToList();
        ...
        public static List<DateTime> GetQAWorkDates(this Team team)
        {
            using (TeamHubDatabaseContext dataContext = new TeamHubDatabaseContext())
            {
                ...
                var teamChanges = dataContext.TeamChanges.Include(tc => tc.TeamMember)
                                                         .ThenInclude(d => d.Specialization).Where(tc => team.GetEmployees().Select(d => d.Id).Contains(tc.TeamMemberId)).ToList();
        ...
        public static List<Transfer> GetTeamChanges(this Team team)
        {
            using (TeamHubDatabaseContext dataContext = new TeamHubDatabaseContext())
            {
                var teamChanges = dataContext.Transfers.Include(tc => tc.Employee)
                                                         .Include(tc => tc.FromTeam)
                                                         .Include(tc => tc.ToTeam).Where(tc => team.GetEmployees().Select(d => d.Id).Contains(tc.EmployeeId)).ToList();
                return teamChanges;
            }
        }
    }
